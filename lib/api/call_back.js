...
module.exports = class CallBack {
  ...
  async decrypt (query, body) {
    const { nonce, timestamp } = query
    const encrypt = this.cryptor.encrypt('success')
    return {
      decrypted: this.cryptor.decrypt(body.encrypt).message,
      success: {
   ...
   * @return {Object} { errcode, errmsg }
   */
  async update (callBackTag, url, opts) {
